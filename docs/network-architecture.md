...
MTU가 어긋나 있을 때(일부 구간 9000 / 일부 1500)는 `iperf3`에서 receiver가 0 bytes, Cwnd가 약 11 KB로 축소되고 재전송이 반복됐습니다. 큰 프레임이 작은 MTU 구간에서 silent drop된 것이 원인이었고, NIC 자체 문제는 `ethtool -S`로 에러 카운터가 0인 것을 확인해 배제했습니다. 이후 전 구간 MTU를 1500으로 일관되게 맞춰 불일치를 해소했습니다. (점보 프레임은 대용량 연속 전송에 이론적 이점이 있으나, 본 환경에선 디스크 처리량이 병목이고 외부 통신·작은 패킷 워크로드엔 이득이 없어, 일관성과 단순성을 위해 표준 1500으로 통일했습니다.)
